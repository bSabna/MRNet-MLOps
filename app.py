...
# Label mapping: matches train_gatekeeper.py (0=Axial, 1=Coronal, 2=Sagittal)
...
gatekeeper.load_state_dict(
    torch.load("gatekeeper_weights.pth", map_location=torch.device("cpu"), weights_only=True)
)
gatekeeper.eval()
...
diagnosis_model = load_production_model("mrnet_3dcnn_artifacts.pth")
print("Diagnosis model loaded.")

# Optimal thresholds derived from Youden's J statistic during training
...
def bytes_to_middle_slice_tensor(file_bytes: bytes) -> torch.Tensor:
    ...
    normalize to [0,1], resize to 256x256, return (1, 1, 256, 256) tensor.
    """
...
def validate_plane(file_bytes: bytes, expected_plane: str) -> tuple[bool, str, float]:
    ...
    return (predicted_plane == expected_plane), predicted_plane, confidence


def build_clinical_summary(results: dict) -> dict:
    """
    Generate a human-readable clinical summary based on prediction probabilities.
    """
    positive_findings = [k for k, v in results.items() if v["positive"]]
    top_finding = max(results.items(), key=lambda x: x[1]["probability"])
    top_name, top_prob = top_finding[0], top_finding[1]["probability"]

    if top_prob > 0.75:
        confidence_level = "High Confidence"
        recommendation = (
            "Strong indication of pathology detected. "
            "Radiologist review is highly recommended."
        )
    elif top_prob > 0.50:
        confidence_level = "Moderate Confidence"
        recommendation = (
            "Possible pathology detected. "
            "Consult a radiologist for clinical confirmation."
        )
    else:
        confidence_level = "Low Confidence / Uncertain"
        recommendation = (
            "Results are inconclusive. "
            "Clinical correlation and radiologist review strongly advised."
        )

    return {
        "primary_finding": top_name,
        "probability": round(top_prob, 4),
        "confidence_level": confidence_level,
        "positive_findings": positive_findings if positive_findings else ["None detected"],
...
async def predict(
    ...
    """
    **Step 1** — Gatekeeper CNN validates each file is the correct anatomical plane.

    **Step 2** — Main 3D CNN runs multi-label diagnosis.

    **Returns** predictions for ACL tear, Meniscus tear, and Abnormality,
    along with a clinical summary and recommendation.
    """
    ...
    for file_bytes, expected_plane, filename in validations:
        ...
        if not is_valid:
            ...
            raise HTTPException(
                ...
                detail=(
                    f"Plane mismatch in '{filename}': "
                    f"Expected '{expected_plane}' but received '{predicted_plane}' "
    ...
    try:
        ...
        return {
            ...
            "predictions": results,
            "clinical_summary": build_clinical_summary(results),
